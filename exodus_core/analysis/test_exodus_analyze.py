...
from exodus_core.analysis.static_analysis import StaticAnalysis, ExodusCoreException
...
class TestInit(unittest.TestCase):
    ...
    def test_raises_exception_if_file_none(self):
        with self.assertRaisesRegex(ExodusCoreException, "file missing"):
            StaticAnalysis()

    def test_raises_exception_if_file_missing(self):
        with self.assertRaisesRegex(ExodusCoreException, "file missing"):
            StaticAnalysis('missing_file.apk')

    def test_object_is_created(self):
        with patch('androguard.core.bytecodes.apk.APK'):
            sa = StaticAnalysis('./apks/francetv.apk')
        assert sa.apk_file_path == './apks/francetv.apk'


class TestGetApkSha256(unittest.TestCase):

    def test_calculates_correct_sha(self):
        sa = StaticAnalysis('./apks/francetv.apk')
        sha = 'b1a17153f52ecec6d7ec15eb60ae546af96e6f9a73d436d72acdf391e026d00b'
        assert sa._get_apk_sha256() == sha


class TestGetIconFromManifest(unittest.TestCase):

    def test_returns_icon_path(self):
        sa = StaticAnalysis('./apks/francetv.apk')
        icon_path = sa._get_icon_from_manifest('icon.png')
        assert icon_path is not None

    def test_raises_exception_if_no_icon(self):
        sa = StaticAnalysis('./apks/francetv.apk')
        with patch.object(APK, 'get_app_icon', return_value=None):
            with self.assertRaisesRegex(ExodusCoreException, 'No icon'):
                sa._get_icon_from_manifest('icon.png')


class TestGetIconFromDetails(unittest.TestCase):

    def test_returns_empty_string_if_exception(self):
        with patch('androguard.core.bytecodes.apk.APK'):
            with patch('urllib.request.urlopen'):
                sa = StaticAnalysis('./apks/francetv.apk')
                icon_path = sa._get_icon_from_details('icon.png')
        assert icon_path == ''

    def test_returns_empty_string_if_no_details(self):
        with patch('androguard.core.bytecodes.apk.APK'):
            with patch('exodus_core.analysis.static_analysis.StaticAnalysis.get_application_details'):
                sa = StaticAnalysis('./apks/francetv.apk')
                icon_path = sa._get_icon_from_details('icon.png')
        assert icon_path == ''


class TestGetIconFromGPlay(unittest.TestCase):

    def test_returns_empty_string_if_exception(self):
        with patch('androguard.core.bytecodes.apk.APK'):
            with patch('urllib.request.urlopen'):
                sa = StaticAnalysis('./apks/francetv.apk')
                icon_path = sa._get_icon_from_gplay('francetv', 'icon.png')
        assert icon_path == ''


class TestGetCertificates(unittest.TestCase):

    def test_calls_function(self):
        sa = StaticAnalysis('./apks/francetv.apk')
        certificates = sa._get_certificates()
        print(certificates)
